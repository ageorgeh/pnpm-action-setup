...
export async function runRestoreCache(inputs: Inputs) {
  ...
  const restoreKeys = [`pnpm-cache-${process.env.RUNNER_OS}-${os.arch()}-`];
...
async function getCacheDirectory() {
  ...
  let directory = cacheFolderPath;
  if (cacheFolderPath.startsWith(`${home}${path.sep}`)) {
    directory = `~/${path.relative(home, cacheFolderPath).split(path.sep).join("/")}`;
  }

  debug(`Cache folder is set to "${directory}"`);
  return directory;
}
